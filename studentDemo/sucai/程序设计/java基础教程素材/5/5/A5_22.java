...
public class A5_22 {
	public static void main(String[] args){
		...
		System.out.println();
		for(int i=0;i<renshu;i++){
			System.out.print(name[i]);//循环输出学生的姓名
			for(int j=0;j<courseNum;j++){
				System.out.print("\t"+number[i][j]);//循环输出学生的各科对应的成绩
			}
			System.out.print("\t"+sum[i]);//循环输出总分
			System.out.print("\t"+avg[i]);//循环输出平均分
			System.out.println();//起换行的作用
		}
	}
}
