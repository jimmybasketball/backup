 ...
 */
@Component("orderClearanceCheckProcess")
public class OrderClearanceCheckProcess extends TemplateProcessor
                                                                 implements
    ...
    public BaseResult process(StockoutOrderRequest request) throws ServiceException {
        LogBetter.instance(logger).setLevel(LogLevel.INFO).setMsg("[供应链]变更出库单状态为清关异常")
        ...
        try {
            ...
            Date overseaStockinAlarmTime = EffectiveAlarmConfig.getAlarmDate(
                ...
                StockoutOrderState.CLEARANCE_CHECKING.getValue());
            stockoutOrderStateLogManager.updateStockOutOrderStateLog(
                StockoutOrderConvert.convertBOToDO(stockoutOrderBO),
                StockoutOrderState.CLEARANCE_CHECKING.getValue(), eventTime,
                overseaStockinAlarmTime, SystemUserName.SYSTEM.getValue());
            ...
            if (request.getOperator() != null && request.getOperator().getName() != null
                ...
                traceLogger.log(new TraceLog(stockoutOrderBO.getBizId(), "supplychain", new Date(),
                    TraceLog.TraceLevel.INFO, "发货单状态变更为[清关异常]"));
                return result;
            }
            ...
            routeService.appendUserRoute(
                ...
                buildLogisticsUserRoute(stockoutOrderBO.getBizId(), "包裹海关扣留查验中",
                    eventTime.getTime()));
            BaseResult result = new BaseResult(true);
            ...
            LogBetter.instance(logger).setLevel(LogLevel.ERROR).setErrorMsg("[供应链]变更出库单状态为清关异常")
                .setException(e).log();
