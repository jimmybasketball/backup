...
public class PdfOrderCreateProcessor {
    ...
    private static final Logger logger = LoggerFactory.getLogger(PdfOrderCreateProcessor.class);
...
//    }
     ...
     */
    private void buildPdfFile(List<ExportedShipOrderDO> exportedShipOrderDOs, String pdfPath, WarehouseDO warehouseDO) throws ServiceException {
        try {
            ...
            for (ExportedShipOrderDO element : exportedShipOrderDOs) {
                ...
                if (PdfTemplateType.valueOf(logisticsLineDO.getPdfTemplate()) == null) {
                    throw new ServiceException(StockoutReturnCode.STOCKOUT_ORDER_SERVICE_PDFTEMPLATE_ERROR, "线路PDFTemplate配置错误");
                }
                ...
                PdfPTable pdfPTable = pdfFormat.buildPdfBill(element, warehouseDO, logisticsLineDO, cb);
                // 判断页的大小
                if(StringUtils.equals(pdfFormatType, PdfTemplateType.RHF_YUNDA.getValue()) || StringUtils.equals(pdfFormatType, PdfTemplateType.SH_YUNDA.getValue())){
                    document.setMargins(0f, 0f, 0f, 0f);
                    document.setPageSize(new RectangleReadOnly(
                            NumberUtil.parseMmToPt(101), NumberUtil.parseMmToPt(203)));
                    ...
                    document.setMargins(0f, 0f, 0f, 0f);
                    document.setPageSize(new RectangleReadOnly(
                            NumberUtil.parseMmToPt(100), NumberUtil.parseMmToPt(149)));
                }else if(StringUtils.equals(pdfFormatType, PdfTemplateType.NEW_ACS.getValue())){
                    document.setMargins(0f, 0f, 0f, 0f);
                    document.setPageSize(new RectangleReadOnly(
                            NumberUtil.parseMmToPt(100), NumberUtil.parseMmToPt(149)));
                }else if(StringUtils.equals(pdfFormatType, PdfTemplateType.ACS_ZTO.getValue()) || StringUtils.equals(pdfFormatType, PdfTemplateType.GAOJIE_YT.getValue())){
                    document.setMargins(0f, 0f, 0f, 0f);
                    document.setPageSize(new RectangleReadOnly(
                            NumberUtil.parseMmToPt(100), NumberUtil.parseMmToPt(150)));
                }else if(StringUtils.equals(pdfFormatType, PdfTemplateType.YXT_ZTO_2.getValue())){
                    document.setMargins(0f, 0f, 0f, 0f);
                    document.setPageSize(new RectangleReadOnly(
                            NumberUtil.parseMmToPt(100), NumberUtil.parseMmToPt(190)));
                }else if(StringUtils.equals(pdfFormatType, PdfTemplateType.YZYW_SIMPLE.getValue())){
                    document.setMargins(5f, 5f, 10f, 10f);
                    document.setPageSize(PageSize.A5);
                    PDFFormat pdfFormats = pdfBillFactory.getPDFFormat(PdfTemplateType.YZYW_SIMPLE_TITLE.getValue());
                    PdfPTable pdfPTables = pdfFormats.buildPdfBill(element, warehouseDO, logisticsLineDO, cb);
                    document.newPage();
            ...
            if (StringUtils.isNotBlank(notSupportPdfOrderIds.toString())) {
                ...
                PDFFormat pdfFormat = pdfBillFactory.getPDFFormat(PdfTemplateType.SIMPLE.getValue());
                PdfPTable pdfPTable = pdfFormat.buildTextOnNewPage(notSupportPdfOrderIds.toString());
                document.setPageSize(PageSize.A5);
                document.newPage();
                document.add(pdfPTable);
            }
