...
/**
 * 杭州优先投清关, 中通配送的PDF面单格式
 * <p>
 * Created by jackiehff on 16/1/7.
 */
@Component("yxtCustomZtoDeliveryPDFFormat")
public class YxtCustomZtoDeliveryPDFFormat extends AbstractPDFFormat {

    private static final Logger logger = LoggerFactory.getLogger(YxtCustomZtoDeliveryPDFFormat.class);
    ...
    public PdfPTable buildPdfBill(ExportedShipOrderDO exportedShipOrderDO, WarehouseDO warehouseDO, LogisticsLineDO logisticsLineDO, PdfContentByte cb) throws ServiceException {
        try {
            ...
            PdfPCell destCodeCell = new PdfPCell(new Paragraph(exportedShipOrderDO.getDestcode(), BoldBig22Chinese));//描述
            destCodeCell.setHorizontalAlignment(Element.ALIGN_CENTER);
            destCodeCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
            destCodeCell.setFixedHeight(30f);
            ...
            cell = new PdfPCell(new Paragraph("杭州市场部", BoldBig16Chinese));//描述
            ...
            cell = new PdfPCell(new Paragraph(userName + "                 " + exportedShipOrderDO.getBuyerTelephone() + "    \n\n"
                    + exportedShipOrderDO.getBuyerAddress(), FontChinese));
            cell.setFixedHeight(45f);
            receiveAndSendTable.addCell(cell);
            ...
            cell = new PdfPCell(new Paragraph(exportedShipOrderDO.getSenderName() + "\n" + exportedShipOrderDO.getSenderTelephone() + "    \n"
                    + exportedShipOrderDO.getSenderAddress(), FontChinese));
            cell.setFixedHeight(40f);
            receiveAndSendTable.addCell(cell);
            ...
            code128Image.scalePercent(150f);//设置缩放的百分比%7.5
            ...
            PdfPCell orderNoCell = new PdfPCell(new Paragraph(" 订单号：" + exportedShipOrderDO.getBizId(), FontChinese));//描述
            orderNoCell.setHorizontalAlignment(Element.ALIGN_LEFT);
            orderNoCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
            orderNoCell.setFixedHeight(15f);
            ...
            code128Image.scalePercent(90f);//设置缩放的百分比%7.5
            ...
            for (int i = 0; i < skuIdList.length; i++) {
                ...
                if (StringUtils.equals("韩国", warehouseDO.getCountry())) {
                    table4Cell = new PdfPCell(new Paragraph(temp, fontSouthKorea));
                } else if (StringUtils.equals("JP", warehouseDO.getCountry())) {
...
//                float[] footerLogoWidth = {90f};// 设置表格的列宽和列数
//                PdfPTable footerLogoTable = new PdfPTable(footerLogoWidth);// 建立一个pdf表格
//                footerLogoTable.setWidthPercentage(100);
//                footerLogoTable.getDefaultCell().setBorder(0);
//
//                PdfPCell footerLogoCell = new PdfPCell(new Paragraph("", BoldBigChinese));//描述
//                footerLogoCell.setHorizontalAlignment(Element.ALIGN_LEFT);// 设置内容水平居中显示
//                footerLogoCell.setVerticalAlignment(Element.ALIGN_BOTTOM);  // 设置垂直居中
//                footerLogoCell.addElement(fqLogo);
//                fqLogo.setWidthPercentage(100);
//                fqLogo.scalePercent(80f);//设置缩放的百分比%7.5
//                footerLogoCell.setFixedHeight(35f);
//                footerLogoCell.setPadding(0);
//                footerLogoCell.setBorder(0);
//
//                footerLogoTable.addCell(footerLogoCell);
//
//                PdfPCell pdfTableCell = new PdfPCell();
//                pdfTableCell.addElement(footerLogoTable);
//                pdfTableCell.setColspan(2);
//
...
//            }
    ...
    public String getFormat() {
        return PdfTemplateType.YXT_ZTO.getValue();
    }
}
