 ...
 * Time: 下午4:06
 */
@Component("simplePDFFormat")
public class SimplePDFFormat extends AbstractPDFFormat {

    private static final Logger logger = LoggerFactory.getLogger(SimplePDFFormat.class);
    ...
    public PdfPTable buildPdfBill(ExportedShipOrderDO element, WarehouseDO warehouseDO, LogisticsLineDO logisticsLineDO, PdfContentByte cb) throws ServiceException {
        try {
            ...
            table3Cell = new PdfPCell(new Paragraph("FQ  出库单号 " + element.getId(), FontChinese));
            table3Cell.setHorizontalAlignment(Element.ALIGN_CENTER);// 设置内容水平居中显示
            table3Cell.setVerticalAlignment(Element.ALIGN_MIDDLE);  // 设置垂直居中
    ...
    public String getFormat() {
        return PdfTemplateType.SIMPLE.getValue();
    }
}
