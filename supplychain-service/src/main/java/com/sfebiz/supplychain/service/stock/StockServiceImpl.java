...
public class StockServiceImpl implements StockService {
    ...
    public boolean incrementSkuBatchStock(long warehouseId, long stockinOrderId, int stockinType,
                                          SkuBatchStockOperaterEntity skuBatchStockOperaterEntity)
                                                                                                  throws ServiceException {
        LogBetter.instance(LOGGER).setLevel(LogLevel.INFO).setMsg("[供应链-增加商品批次库存]")
            .addParm("商品信息", skuBatchStockOperaterEntity).addParm("仓库ID", warehouseId)
            .addParm("入库单ID", stockinOrderId).addParm("入库单类型", stockinType).log();

        if (0 == warehouseId || 0 == stockinOrderId || null == skuBatchStockOperaterEntity
            || 0 == skuBatchStockOperaterEntity.getSkuId()
            || skuBatchStockOperaterEntity.getCount() < 0
            || skuBatchStockOperaterEntity.getWearCount() < 0
            || skuBatchStockOperaterEntity.getPrice() < 0
            || skuBatchStockOperaterEntity.getPriceRmb() < 0) {
            throw new ServiceException(LogisticsReturnCode.STOCK_SERVICE_PARAMS_ILLEGAL,
                "[供应链-增加商品库存失败]: " + LogisticsReturnCode.STOCK_SERVICE_PARAMS_ILLEGAL.getDesc()
                        + " " + "[商品信息: " + skuBatchStockOperaterEntity + ", 仓库ID: " + warehouseId
                        + ", 入库单ID: " + stockinOrderId + "]");
        }

        try {
            //1. 获取实物库存记录，如果不存在则创建
            StockPhysicalDO stockPhysicalDO = getStockPhysicalBySkuIdAndwarehouseIdIfNotExistThenCreate(
                skuBatchStockOperaterEntity.getSkuId(), warehouseId);
            //2. 更新实物库存的信息和成本价
            return updateSkuBatchStockCount(stockPhysicalDO, stockinOrderId, stockinType,
                skuBatchStockOperaterEntity);
        } catch (ServiceException e) {
            LogBetter.instance(LOGGER).setLevel(LogLevel.WARN).setErrorMsg(e.getMessage())
                .setException(e).log();
            throw e;
        } catch (Exception e) {
            LogBetter.instance(LOGGER).setLevel(LogLevel.ERROR)
                ...
                .addParm("商品信息", skuBatchStockOperaterEntity).addParm("仓库ID", warehouseId)
                .addParm("入库单ID", stockinOrderId).setException(e).log();
            throw new ServiceException(LogisticsReturnCode.STOCK_SERVICE_INNER_EXCEPTION,
                "[供应链-增加商品库存异常]: " + e.getMessage() + " " + "[商品信息: " + skuBatchStockOperaterEntity
                        + ", 仓库ID: " + warehouseId + ", 入库单ID: " + stockinOrderId + "]");
        }
    }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public Map<String, List<SkuBatchStockOperaterEntity>> freezeSkuStockBatch(List<SkuStockOperaterEntity> skuStockOperaterEntityList)
                                                                                                                                      throws ServiceException {
        LogBetter.instance(LOGGER).setLevel(LogLevel.INFO).setMsg("[供应链-批量冻结库存信息]:开始")
            .addParm("商品信息", skuStockOperaterEntityList).log();
        //1. 判断参数合法性
        if (null == skuStockOperaterEntityList || skuStockOperaterEntityList.size() == 0) {
            throw new ServiceException(LogisticsReturnCode.STOCK_SERVICE_PARAMS_ILLEGAL,
                "[供应链-批量冻结库存信息失败]: " + LogisticsReturnCode.STOCK_SERVICE_PARAMS_ILLEGAL.getDesc()
        ...
        for (SkuStockOperaterEntity skuStockOperaterEntity : mergeAndSortResult) {
            ...
            //4.2 冻结库存
            checkAndFreezeSkuBatchStock(skuStockOperaterEntity.getSkuId(),
                skuStockOperaterEntity.getWarehouseId(), skuStockOperaterEntity.getCount(),
                skuStockOperaterEntity.getStockoutOrderId(), skuBatchStockOperaterEntities);
            //4.3 将商品冻结的批次库存信息返回
            String key = skuStockOperaterEntity.getStockoutOrderId() + "_"
                         + skuStockOperaterEntity.getSkuId();
        ...
        LogBetter.instance(LOGGER).setLevel(LogLevel.INFO).setMsg("[供应链-批量冻结库存信息]:结束")
     ...
     */
    @Override
    public boolean consumeSkuStockInBatch(List<SkuStockOperaterEntity> skuStockOperaterEntityList,
                                          long warehouseId, long stockoutOrderId)
                                                                                 throws ServiceException {
        LogBetter.instance(LOGGER).setLevel(LogLevel.INFO).setMsg("[供应链-批量消费商品库存]")
            .addParm("商品信息", skuStockOperaterEntityList).addParm("仓库ID", warehouseId)
            .addParm("出库单ID", stockoutOrderId).log();
        //1. 判断参数合法性
        if (0 == warehouseId || 0 == stockoutOrderId || null == skuStockOperaterEntityList
            || skuStockOperaterEntityList.size() == 0) {
            throw new ServiceException(LogisticsReturnCode.STOCK_SERVICE_PARAMS_ILLEGAL,
                "[供应链-批量消费商品库存]: " + LogisticsReturnCode.STOCK_SERVICE_PARAMS_ILLEGAL.getDesc()
                        + "[商品信息: " + skuStockOperaterEntityList + ", 仓库ID: " + warehouseId
        ...
        for (SkuStockOperaterEntity skuStockOperaterEntity : mergeAndSortResult) {
            consumeSkuStock(skuStockOperaterEntity.getSkuId(), warehouseId,
                skuStockOperaterEntity.getCount(), stockoutOrderId);
        }
     ...
     */
    @Override
    public boolean releaseSkuStockInBatch(List<SkuStockOperaterEntity> skuStockOperaterEntityList,
                                          long warehouseId, long stockoutOrderId)
                                                                                 throws ServiceException {
        LogBetter.instance(LOGGER).setLevel(LogLevel.INFO).setMsg("[供应链-批量释放商品库存]")
            .addParm("商品信息", skuStockOperaterEntityList).addParm("仓库ID", warehouseId)
            .addParm("出库单ID", stockoutOrderId).log();
        if (0 == warehouseId || 0 == stockoutOrderId || null == skuStockOperaterEntityList
            || skuStockOperaterEntityList.size() == 0) {
        ...
        for (SkuStockOperaterEntity skuStockOperaterEntity : mergeAndSortResult) {
            releaseSkuStock(skuStockOperaterEntity.getSkuId(), warehouseId,
                skuStockOperaterEntity.getCount(), stockoutOrderId);
        }
        ...
        LogBetter.instance(LOGGER).setLevel(LogLevel.INFO).setMsg("[供应链-批量释放商品库存成功]")
            .addParm("商品信息", skuStockOperaterEntityList).addParm("仓库ID", warehouseId)
            .addParm("出库单ID", stockoutOrderId).log();
        return true;
    }
     ...
     * @throws ServiceException
     */
    protected StockPhysicalDO getStockPhysicalBySkuIdAndwarehouseIdIfNotExistThenCreate(long skuId,
                                                                                        long warehouseId)
                                                                                                         throws ServiceException {
        StockPhysicalDO stockPhysicalDO = stockPhysicalManager.getBySkuIdAndWarehouseId(skuId,
            warehouseId);
     ...
     * @throws ServiceException
     */
    protected boolean updateSkuBatchStockCount(StockPhysicalDO stockPhysicalDO,
                                               long stockinOrderId,
                                               int stockinType,
                                               SkuBatchStockOperaterEntity skuBatchStockOperaterEntity)
                                                                                                       throws ServiceException {
        LogBetter.instance(LOGGER).setMsg("[供应链-增加商品库存]: 更新商品批次库存的数量和价格")
            .addParm("商品信息", skuBatchStockOperaterEntity).addParm("入库单ID", stockinOrderId)
            .addParm("入库单类型", stockinType).log();
        ...
        LogBetter.instance(LOGGER).setLevel(LogLevel.INFO).setMsg("[供应链-增加商品库存成功]")
            .addParm("商品信息", skuBatchStockOperaterEntity).addParm("入库单ID", stockinOrderId).log();
        return true;
    }
     ...
     * @throws ServiceException
     */
    private void addBatchStock(StockPhysicalDO stockPhysicalDO, long stockinOrderId,
                               int stockinType,
                               SkuBatchStockOperaterEntity skuBatchStockOperaterEntity)
                                                                                       throws ServiceException {
        //增加sku批次库存
        StockBatchDO existBatchStockDO = stockBatchManager.getBySkuIdAndWarehouseIdAndBatchNo(
            stockPhysicalDO.getSkuId(), stockPhysicalDO.getWarehouseId(),
            skuBatchStockOperaterEntity.getBatchNo());
        if (existBatchStockDO == null) {
            ...
            if (stockinType == StockinOrderType.SALES_STOCK_IN.getValue()) {
                throw new ServiceException(
                    ...
                    "[供应链-供应链-增加商品库存异常: ]"
                            + LogisticsReturnCode.STOCK_SERVICE_BATCH_STOCK_RECODE_DUPLICATE
                                .getDesc() + " " + "[商品信息: " + skuBatchStockOperaterEntity
                            + ", 入库单ID: " + stockinOrderId + "]");
            } else {
                existBatchStockDO.setAvailableCount(existBatchStockDO.getAvailableCount()
                    .intValue() + skuBatchStockOperaterEntity.getCount().intValue());
                existBatchStockDO.setDamagedCount(existBatchStockDO.getDamagedCount()
                                                  + skuBatchStockOperaterEntity.getWearCount());
                existBatchStockDO
                    .setExpirationDate(skuBatchStockOperaterEntity.getExpirationDate());
                existBatchStockDO
                    .setProductionDate(skuBatchStockOperaterEntity.getProductionDate());
    ...
    protected List<SkuStockOperaterEntity> mergeAndSortSkuStockOperaterEntityBySkuIdAndStockoutOrderId(List<SkuStockOperaterEntity> souceEntityList) {
        ...
        for (SkuStockOperaterEntity skuStockOperaterEntity : souceEntityList) {
            String key = skuStockOperaterEntity.getSkuId() + "_"
                         + skuStockOperaterEntity.getStockoutOrderId();
            if (map.containsKey(key)) {
                SkuStockOperaterEntity oldSkuStockOperaterEntity = map.get(key);
                oldSkuStockOperaterEntity.setCount(oldSkuStockOperaterEntity.getCount()
                                                   + skuStockOperaterEntity.getCount());
                oldSkuStockOperaterEntity.setWearCount(oldSkuStockOperaterEntity.getWearCount()
                                                       + skuStockOperaterEntity.getWearCount());
                oldSkuStockOperaterEntity.setOverFlowCount(oldSkuStockOperaterEntity
                    .getOverFlowCount() + skuStockOperaterEntity.getOverFlowCount());
                oldSkuStockOperaterEntity.setOverFlowWearCount(oldSkuStockOperaterEntity
                    .getOverFlowWearCount() + skuStockOperaterEntity.getOverFlowWearCount());
            } else {
    ...
    protected List<SkuBatchStockOperaterEntity> checkAndGetNeedFreezeBatchStockByWarehouse(SkuStockOperaterEntity skuStockOperaterEntity)
                                                                                                                                         throws ServiceException {
        long warehouseId = skuStockOperaterEntity.getWarehouseId();
        //1. 获取仓库中所有符合要求的批次信息
        List<StockBatchDO> stockBatchDOList = stockBatchManager.getBySkuIdAndWarehouseId(
            skuStockOperaterEntity.getSkuId(), warehouseId);
        ...
        if (stockBatchDOList == null || stockBatchDOList.size() == 0) {
            LogBetter
                ...
                .setErrorMsg(
                    "[供应链-根据商品ID和仓库ID获取批次库存信息失败]: "
                            + LogisticsReturnCode.STOCK_SERVICE_BATCH_STOCK_RECORD_NOT_FOUND
                                .getDesc()).addParm("商品ID", skuStockOperaterEntity.skuId)
                .addParm("仓库ID", warehouseId).log();
            throw new ServiceException(
                LogisticsReturnCode.STOCK_SERVICE_BATCH_STOCK_RECORD_NOT_FOUND,
                "[供应链-根据商品ID和仓库ID获取批次库存信息失败]: "
                        + LogisticsReturnCode.STOCK_SERVICE_BATCH_STOCK_RECORD_NOT_FOUND.getDesc()
                        + "[商品ID:" + skuStockOperaterEntity.skuId + ",仓库ID:" + warehouseId + "]");
        }
        ...
        //4. 根据订单数量确定出库批次信息
        List<SkuBatchStockOperaterEntity> freezeBatchStocks = getNeedFreezeBatchStock(sortRusults,
            skuStockOperaterEntity);
    ...
    protected List<SkuBatchStockOperaterEntity> getNeedFreezeBatchStock(List<StockBatchDO> sortRusults,
                                                                        SkuStockOperaterEntity skuStockOperaterEntity)
                                                                                                                      throws ServiceException {
        //1.初始化需要冻结库存信息集合
        ...
        for (StockBatchDO stockBatchDO : sortRusults) {
            LogBetter.instance(LOGGER).setLevel(LogLevel.INFO).setMsg("[供应链-根据商品出库规则计算所需冻结批次库存]")
                .addParm("批次库存信息", stockBatchDO).addParm("仓库剩余需冻结数量", remainNeedCount).log();
        ...
        LogBetter
            ...
            .setLevel(LogLevel.WARN)
            .setErrorMsg(
                "[供应链-根据商品出库规则计算所需冻结批次库存失败]"
                        + LogisticsReturnCode.LOGISTICS_BATCH_STOCK_SKU_NOT_ENOUGH.getDesc())
            .addParm("商品ID", skuStockOperaterEntity.getSkuId())
            .addParm("仓库ID", sortRusults.get(0).getWarehouseId())
            .addParm("商品冻结数量", skuStockOperaterEntity.getCount()).log();
        throw new ServiceException(LogisticsReturnCode.LOGISTICS_BATCH_STOCK_SKU_NOT_ENOUGH,
            "[供应链-根据商品出库规则计算所需冻结批次库存失败]: "
                    + LogisticsReturnCode.LOGISTICS_BATCH_STOCK_SKU_NOT_ENOUGH.getDesc() + "[商品ID:"
                    + skuStockOperaterEntity.skuId + ",仓库ID:" + sortRusults.get(0).getWarehouseId()
    ...
    protected SkuBatchStockOperaterEntity convertFromBatchStockDO(StockBatchDO stockBatchDO) {
        ...
        skuBatchStockOperaterEntity.setFreezeCount(stockBatchDO.getAvailableCount()
                                                   + stockBatchDO.getFreezeCount());
    ...
    protected boolean checkAndFreezeSkuBatchStock(long skuId,
                                                  long warehouseId,
                                                  int count,
                                                  long stockoutOrderId,
                                                  List<SkuBatchStockOperaterEntity> skuBatchStockOperaterEntities)
                                                                                                                  throws ServiceException {
        if (0 == skuId || 0 == warehouseId || 0 == stockoutOrderId || 0 == count
            || skuBatchStockOperaterEntities == null || skuBatchStockOperaterEntities.size() == 0) {
            throw new ServiceException(LogisticsReturnCode.STOCK_SERVICE_PARAMS_ILLEGAL,
                "[供应链-根据需要冻结的批次库存信息检查并冻结商品库存失败]: "
                        + LogisticsReturnCode.STOCK_SERVICE_PARAMS_ILLEGAL.getDesc() + "[商品ID: "
                        + skuId + ", 仓库ID: " + warehouseId + ", 商品数目: " + count + ", 出库单ID: "
                        + stockoutOrderId + ", 批次库存信息: " + skuBatchStockOperaterEntities + "]");
        }

        LogBetter.instance(LOGGER).setLevel(LogLevel.INFO).setMsg("[供应链-根据需要冻结的批次库存信息检查并冻结商品库存]")
            .addParm("商品ID", skuId).addParm("仓库ID", warehouseId).addParm("商品数目", count)
            .addParm("出库单ID", stockoutOrderId)
            .addParm("批次库存数量", skuBatchStockOperaterEntities.size()).log();
        try {
            //1. 检查此销售单是否已经有冻结库存
            List<StockFreezeDO> freezeStockList = getStockoutFreezeStockList(skuId, warehouseId,
                stockoutOrderId);
            if (null != freezeStockList && freezeStockList.size() > 0) {
                LogBetter.instance(LOGGER).setLevel(LogLevel.WARN)
                    .setMsg("[供应链-检查并冻结商品库存]: 该订单的冻结库存记录已存在").addParm("商品ID", skuId)
                    .addParm("仓库ID", warehouseId).addParm("商品数目", count)
                    .addParm("出库单ID", stockoutOrderId).addParm("冻结记录数量", freezeStockList.size())
                    .log();
                return true;
            }
            //2. 判断现有库存是否满足需求，如果满足，则冻结库存
            return checkStockIsEnoughThenFreezeByNeedFreezeBatchStock(skuId, warehouseId,
                stockoutOrderId, count, skuBatchStockOperaterEntities);
        } catch (ServiceException e) {
            throw e;
        } catch (Exception e) {
            LogBetter.instance(LOGGER)
                ...
                .addParm("商品ID", skuId).addParm("仓库ID", warehouseId).addParm("商品数目", count)
                .addParm("出库单ID", stockoutOrderId).setException(e).log();
            throw new ServiceException(LogisticsReturnCode.STOCK_SERVICE_INNER_EXCEPTION,
                "[供应链-检查并冻结商品库存异常]: " + e.getMessage() + "[商品ID: " + skuId + ", 仓库ID: "
                        + warehouseId + ", 商品数目: " + count + ", 出库单ID: " + stockoutOrderId + "]");
        }
    }

    protected List<StockFreezeDO> getStockoutFreezeStockList(long skuId, long warehouseId,
                                                             long stockoutOrderId)
                                                                                  throws ServiceException {
        return stockFreezeManager.getBySkuIdAndwarehouseIdAndStockoutOrderId(skuId, warehouseId,
            stockoutOrderId);
    }

    protected boolean checkStockIsEnoughThenFreezeByNeedFreezeBatchStock(long skuId,
                                                                         long warehouseId,
                                                                         long stockoutOrderId,
                                                                         int count,
                                                                         List<SkuBatchStockOperaterEntity> skuBatchStockOperaterEntities)
                                                                                                                                         throws ServiceException {
        LogBetter.instance(LOGGER).setLevel(LogLevel.INFO).setMsg("[供应链-冻结商品库存]")
            .addParm("商品ID", skuId).addParm("仓库ID", warehouseId).addParm("出库单ID", stockoutOrderId)
            .addParm("需冻结数量", count).log();
        //1.查询商品实物库存信息
        StockPhysicalDO stockPhysicalDO = stockPhysicalManager.getBySkuIdAndWarehouseId(skuId,
            warehouseId);

        LogBetter.instance(LOGGER).setLevel(LogLevel.INFO).setMsg("[供应链-多线程获取后的实物库存信息]")
            .addParm("实物库存信息", stockPhysicalDO).log();
        ...
        if (stockPhysicalDO.getAvailableCount() >= count) {
            ...
            LogBetter.instance(LOGGER).setLevel(LogLevel.INFO).setMsg("[供应链-冻结商品实物库存成功]")
                .addParm("商品ID", skuId).addParm("仓库ID", warehouseId)
                .addParm("出库单ID", stockoutOrderId).addParm("需冻结数量", count)
                .addParm("实物库存信息", stockPhysicalDO).log();
        } else {
            LogBetter.instance(LOGGER).setLevel(LogLevel.ERROR)
            ...
            throw new ServiceException(LogisticsReturnCode.LOGISTICS_STOCK_SKU_NOT_ENOUGH,
                "[供应链-冻结商品实物库存失败]: " + LogisticsReturnCode.LOGISTICS_STOCK_SKU_NOT_ENOUGH.getDesc()
                        + ", [商品ID: " + skuId + ", 仓库ID: " + warehouseId + ", 出库单ID: "
                        + stockoutOrderId + ", 需冻结数量: " + count + ", 实物库存信息: " + stockPhysicalDO
                        + "]");
        }
        //3. 冻结冻结批次库存记录
        for (SkuBatchStockOperaterEntity skuBatchStockOperaterEntity : skuBatchStockOperaterEntities) {
            ...
            stockFreezeDO.setOrderType(Integer.parseInt(StockFreezeOrderType.SALE_OUT_ORDER_TYPE
                .getValue()));
            ...
            LogBetter.instance(LOGGER).setLevel(LogLevel.INFO).setMsg("[供应链-冻结商品批次库存成功]")
                .addParm("商品ID", skuId).addParm("仓库ID", warehouseId)
                .addParm("出库单ID", stockoutOrderId).addParm("需冻结数量", count)
                .addParm("冻结批次库存信息", skuBatchStockOperaterEntity).log();
        }
        return true;
    }
    ...
    protected List<SkuStockOperaterEntity> mergeAndSortSkuStockOperaterEntity(List<SkuStockOperaterEntity> souceEntityList) {
        ...
        for (SkuStockOperaterEntity skuStockOperaterEntity : souceEntityList) {
            if (map.containsKey(skuStockOperaterEntity.getSkuId())) {
                SkuStockOperaterEntity oldSkuStockOperaterEntity = map.get(skuStockOperaterEntity
                    .getSkuId());
                oldSkuStockOperaterEntity.setCount(oldSkuStockOperaterEntity.getCount()
                                                   + skuStockOperaterEntity.getCount());
                oldSkuStockOperaterEntity.setWearCount(oldSkuStockOperaterEntity.getWearCount()
    ...
    protected boolean consumeSkuStock(long skuId, long warehouseId, int count, long stockoutOrderId)
                                                                                                    throws ServiceException {
        LogBetter.instance(LOGGER).setLevel(LogLevel.INFO).setMsg("[供应链-消费商品库存]")
            .addParm("商品ID", skuId).addParm("仓库ID", warehouseId).addParm("商品数目", count)
            .addParm("出库单ID", stockoutOrderId).log();
        //1.校验参数合法性
        if (0 == skuId || 0 == warehouseId || 0 == stockoutOrderId || 0 == count) {
            throw new ServiceException(LogisticsReturnCode.STOCK_SERVICE_PARAMS_ILLEGAL,
                "[供应链-消费商品库存失败]: " + LogisticsReturnCode.STOCK_SERVICE_PARAMS_ILLEGAL.getDesc()
                        + "[商品ID: " + skuId + ", 仓库ID: " + warehouseId + ", 商品数目: " + count
                        + ", 出库单ID: " + stockoutOrderId + "]");
        }
        try {
            //2. 查找待消费的冻结库存
            ...
            LogBetter.instance(LOGGER).setLevel(LogLevel.ERROR)
                ...
                .addParm("出库单ID", stockoutOrderId).setException(e).log();
            throw new ServiceException(LogisticsReturnCode.STOCK_SERVICE_INNER_EXCEPTION,
                "[供应链-消费商品库存异常]: " + e.getMessage() + "[商品ID: " + skuId + ", 仓库ID: " + warehouseId
                        + ", 商品数目: " + count + ", 出库单ID: " + stockoutOrderId + "]");
        }
    }

    protected void consumeFreezeStock(long skuId, long warehouseId,
                                      List<StockFreezeDO> stockFreezeDOS, int count)
                                                                                    throws ServiceException {
        //1.校验实物库存
        StockPhysicalDO stockPhysicalDO = stockPhysicalManager.getBySkuIdAndWarehouseId(skuId,
            warehouseId);
        if (stockPhysicalDO == null) {
            LogBetter.instance(LOGGER).setLevel(LogLevel.ERROR)
                ...
                .addParm("商品ID", skuId).addParm("仓库ID", warehouseId).log();
            throw new ServiceException(LogisticsReturnCode.STOCK_SERVICE_ACTUAL_RECORD_NOT_FOUND,
                "[供应链-查找商品库存失败]: "
                        + LogisticsReturnCode.STOCK_SERVICE_ACTUAL_RECORD_NOT_FOUND.getDesc()
                        + "[商品ID: " + skuId + ", 仓库ID: " + warehouseId + "]");
        }
        //2.更新实物库存的数量
        stockPhysicalDO.setFreezeCount(stockPhysicalDO.getFreezeCount() - count);
        stockPhysicalManager.update(stockPhysicalDO);
        ...
        for (StockFreezeDO freezeStockDO : stockFreezeDOS) {
            //3.1更新批次库存信息
            StockBatchDO stockBatchDO = stockBatchManager.getBySkuIdAndWarehouseIdAndBatchNo(skuId,
                warehouseId, freezeStockDO.getBatchNo().trim());
            if (stockBatchDO == null) {
                LogBetter
                    ...
                    .setErrorMsg(
                        LogisticsReturnCode.STOCK_SERVICE_BATCH_STOCK_RECORD_NOT_FOUND.getDesc())
                    .addParm("商品ID", skuId).addParm("仓库ID", warehouseId)
                    .addParm("冻结库存信息", freezeStockDO).log();
                throw new ServiceException(
                    LogisticsReturnCode.STOCK_SERVICE_BATCH_STOCK_RECORD_NOT_FOUND,
                    "[供应链-查找商品库存失败]: "
                            + LogisticsReturnCode.STOCK_SERVICE_BATCH_STOCK_RECORD_NOT_FOUND
                                .getDesc() + "[商品ID: " + skuId + ", 仓库ID: " + warehouseId
                            + ", 冻结库存信息: " + freezeStockDO + "]");
            }
            stockBatchDO.setFreezeCount(stockBatchDO.getFreezeCount()
                                        - freezeStockDO.getFreezeCount());
            stockBatchManager.update(stockBatchDO);
            //3.2更新冻结库存的状态
            ...
            LogBetter.instance(LOGGER).setLevel(LogLevel.INFO).setMsg("[供应链-消费商品库存成功]")
                .addParm("商品ID", skuId).addParm("仓库ID", warehouseId)
                .addParm("冻结库存信息", freezeStockDO).log();
        }
    }

    protected boolean checkFreezeStockInfoIsCorrect(List<StockFreezeDO> freezeStockList, int count,
                                                    StockFreezeState toState)
                                                                             throws ServiceException {
        if (null == freezeStockList || freezeStockList.size() == 0) {
            LogBetter.instance(LOGGER).setLevel(LogLevel.WARN)
                .setErrorMsg(LogisticsReturnCode.STOCK_SERVICE_FREEZE_RECORD_NOT_FOUND.getDesc())
                .addParm("freezeStockList", freezeStockList).addParm("count", count)
                .addParm("toState", toState).log();
        ...
        for (StockFreezeDO freezeStockDO : freezeStockList) {
            if (!freezeStockDO.getFreezeState().equals(StockFreezeState.FREEZED.getValue())) {
                LogBetter
                    ...
                    .setLevel(LogLevel.ERROR)
                    .setErrorMsg(
                        LogisticsReturnCode.STOCK_SERVICE_FREEZE_RECORD_STATE_ERROR.getDesc())
                    .addParm("freezeStockDO", freezeStockDO).addParm("count", count)
                    .addParm("toState", toState).log();
                throw new ServiceException(
                    LogisticsReturnCode.STOCK_SERVICE_FREEZE_RECORD_STATE_ERROR,
                    "[供应链-商品冻结库存状态异常: ]"
                            + LogisticsReturnCode.STOCK_SERVICE_FREEZE_RECORD_STATE_ERROR.getDesc()
                            + "[冻结库存记录: " + freezeStockDO + ", 商品数目: " + count + ", 转换状态: "
                            + toState + "]");
            }
            LogBetter.instance(LOGGER).setLevel(LogLevel.INFO).setMsg("[供应链-检查商品冻结状态 ]")
                .addParm("freezeStockDO", freezeStockDO).addParm("count", count)
                .addParm("toState", toState).log();
        ...
        if (totalFreezeCount != count) {
            LogBetter.instance(LOGGER).setLevel(LogLevel.WARN).setErrorMsg("实际消费库存和冻结库存不符")
                .addParm("totalFreezeCount", totalFreezeCount).addParm("count", count)
                .addParm("toState", toState).log();
            throw new ServiceException(LogisticsReturnCode.STOCK_SERVICE_FREEZE_COUNT_NOT_MATCH,
                "[供应链-商品冻结库存状态异常: ]"
                        + LogisticsReturnCode.STOCK_SERVICE_FREEZE_COUNT_NOT_MATCH.getDesc()
                        + "[冻结库存总数: " + totalFreezeCount + ", 商品数目: " + count + ", 转换状态: "
                        + toState + "]");
        }
        return true;
    }

    private boolean releaseSkuStock(Long skuId, long warehouseId, Integer count,
                                    long stockoutOrderId) throws ServiceException {
        LogBetter.instance(LOGGER).setLevel(LogLevel.INFO).setMsg("[供应链-释放商品库存]")
            .addParm("商品ID", skuId).addParm("仓库ID", warehouseId).addParm("商品数目", count)
            .addParm("出库单ID", stockoutOrderId).log();
        if (0 == skuId || 0 == warehouseId || 0 == stockoutOrderId || 0 == count) {
            throw new ServiceException(LogisticsReturnCode.STOCK_SERVICE_PARAMS_ILLEGAL,
        ...
        try {
            //1. 查找待释放的冻结库存
            List<StockFreezeDO> stockFreezeDOList = getStockoutFreezeStockList(skuId, warehouseId,
                stockoutOrderId);
            ...
            LogBetter.instance(LOGGER).setMsg("[供应链-释放商品库存失败]" + e.getMessage())
                .addParm("商品ID", skuId).addParm("仓库ID", warehouseId).addParm("商品数目", count)
                .addParm("出库单ID", stockoutOrderId).setException(e).log();
            throw new ServiceException(LogisticsReturnCode.STOCK_SERVICE_INNER_EXCEPTION,
                "[供应链-释放商品库存失败]: " + e.getMessage() + "[商品ID: " + skuId + ", 仓库ID: " + warehouseId
                        + ", 商品数目: " + count + ", 出库单ID: " + stockoutOrderId + "]");
        }
    }

    protected void releaseFreezeStock(long skuId, long warehouseId,
                                      List<StockFreezeDO> freezeStockDOs, int count)
                                                                                    throws ServiceException {
        //1.校验实物库存和仓库信息
        StockPhysicalDO stockPhysicalDO = stockPhysicalManager.getBySkuIdAndWarehouseId(skuId,
            warehouseId);
        ...
        for (StockFreezeDO stockFreezeDO : freezeStockDOs) {
            //3.1更新批次库存信息
            StockBatchDO batchStockDO = stockBatchManager.getBySkuIdAndWarehouseIdAndBatchNo(skuId,
                warehouseId, stockFreezeDO.getBatchNo());
            ...
            if (batchStockDO == null) {
                LogBetter
                    ...
                    .setErrorMsg(
                        LogisticsReturnCode.STOCK_SERVICE_BATCH_STOCK_RECORD_NOT_FOUND.getDesc())
                    .addParm("商品ID", skuId).addParm("仓库ID", warehouseId)
                    .addParm("冻结库存信息", batchStockDO).log();
                throw new ServiceException(
                    LogisticsReturnCode.STOCK_SERVICE_BATCH_STOCK_RECORD_NOT_FOUND,
                    "[供应链-查找商品库存失败]: "
                            + LogisticsReturnCode.STOCK_SERVICE_BATCH_STOCK_RECORD_NOT_FOUND
            ...
            batchStockDO.setAvailableCount(batchStockDO.getAvailableCount()
                                           + stockFreezeDO.getFreezeCount());
            batchStockDO.setFreezeCount(batchStockDO.getFreezeCount()
                                        - stockFreezeDO.getFreezeCount());
            ...
            LogBetter.instance(LOGGER).setLevel(LogLevel.INFO).setMsg("[供应链-释放商品库存成功]")
                .addParm("商品ID", skuId).addParm("仓库ID", warehouseId)
                .addParm("冻结库存信息", stockFreezeDO).log();
        }
    }
     ...
     * @throws ServiceException
     */
    private void updateStockPhysicalCount(StockPhysicalDO stockPhysicalDO, long stockinOrderId,
                                          SkuBatchStockOperaterEntity skuBatchStockOperaterEntity)
                                                                                                  throws ServiceException {
        LogBetter.instance(LOGGER).setLevel(LogLevel.INFO).setMsg("[供应链-增加商品库存]: 更新实物库存数量")
            .addParm("商品信息", skuBatchStockOperaterEntity).addParm("入库单ID", stockinOrderId).log();
        // 更新实物库存的破损库存
        stockPhysicalDO.setDamagedCount(stockPhysicalDO.getDamagedCount()
                                        + skuBatchStockOperaterEntity.getWearCount());
        stockPhysicalDO.setAvailableCount(stockPhysicalDO.getAvailableCount()
                                          + skuBatchStockOperaterEntity.getCount());
