 ...
 */
@Component("orderClearanceDoProcessor")
public class OrderClearanceDoProcessor extends TemplateProcessor
                                                                implements
    ...
    public BaseResult process(StockoutOrderRequest request) throws ServiceException {
        LogBetter.instance(logger).setLevel(LogLevel.INFO).setMsg("[供应链]变更出库单状态为清关开始")
        ...
        try {
            ...
            stockoutOrderStateLogManager.updateStockOutOrderStateLog(
                ...
                StockoutOrderState.CLEARANCE_BEGIN.getValue(), eventTime, overseaStockinAlarmTime,
            ...
            if (request.getOperator() != null && request.getOperator().getName() != null
                ...
                traceLogger.log(new TraceLog(stockoutOrderBO.getBizId(), "supplychain", new Date(),
                    TraceLog.TraceLevel.INFO, "发货单状态变更为[清关开始]"));
            ...
            routeService.appendUserRoute(
                ...
                buildLogisticsUserRoute(stockoutOrderBO.getBizId(), "包裹开始清关，如遇海关查验可能会持续较长时间，请耐心等待",
                    eventTime.getTime()));
            ...
            LogBetter.instance(logger).setLevel(LogLevel.ERROR).setErrorMsg("[供应链]变更出库单状态为清关开始异常")
                .setException(e).log();
