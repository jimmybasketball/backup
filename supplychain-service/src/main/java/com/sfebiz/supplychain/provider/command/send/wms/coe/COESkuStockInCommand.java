 ...
 * Time: 上午10:09
 */
public class COESkuStockInCommand extends WmsOrderSkuStockInCommand {
...
//        logisticsOrder.setCarrierCode(stockinOrderBO.getCarrierCode());
        ...
        if (getStockinOrderBO().getType().equals(StockinOrderType.SALES_STOCK_IN.getValue())) {
            ...
            logisticsOrder.setSupplierId(stockinOrderBO.getMerchantProviderId().intValue());
        }
...
//            // 海关退货，顾客退货，对于COE仓库而言都属于退货，相同处理逻辑，沿用消息 入库类型：0-采购 1-转运 2-调拨 3-退货
...
//            sku.lastSendBatchNo = stockinOrderDetailDO.getLastSendSkuBatch();
