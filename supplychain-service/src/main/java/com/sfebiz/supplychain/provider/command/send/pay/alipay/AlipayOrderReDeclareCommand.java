 ...
 * Created by yanmingming on 2015/5/11.
 */
public class AlipayOrderReDeclareCommand extends PayOrderReDeclareCommand {
    ...
    private String buildRequest() throws Exception {
        ...
        request.append("&sign=");
        request.append(generateRequestSign(portMeta, userOrderTotal, requestNo));
     ...
     * @return
     */
    private String generateRequestSign(Map<String, Object> portMeta, String amount, String requestNo) {
        ...
        parameters.add("trade_no=" + stockoutOrderBO.getMerchantPayNo());
                ...
                if (alipayBillDeclareResponse.getResponseContentWrap() != null
                    ...
                    if ("SUCCESS".equalsIgnoreCase(responseContent.getResultCode())) {
                        ...
                        LogBetter.instance(logger)
                                ...
                                .setMsg("[供应链报文-向支付宝支付重新下发支付单申报指令成功]")
                        ...
                        LogBetter.instance(logger)
                                .setLevel(LogLevel.INFO)
                                .setMsg("[供应链报文-向支付宝支付重新下发支付单申报指令失败]")
                                .addParm("请求报文", requestBody)
                                .addParm("回复报文", response)
                                .log();

                        traceLogger.log(new TraceLog(stockoutOrderBO.getBizId(), "supplychain", new Date(), TraceLog.TraceLevel.INFO,
                                "[供应链报文-向支付宝支付重新下发支付单申报指令失败]: "
                                        + "[请求报文: " + requestBody
                                        + ", 回复报文: " + response
                                        + "]"
                        ));
