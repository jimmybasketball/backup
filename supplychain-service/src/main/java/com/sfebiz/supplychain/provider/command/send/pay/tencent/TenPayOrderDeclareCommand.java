 ...
 */
public class TenPayOrderDeclareCommand extends PayOrderDeclareCommand {
    ...
    private void buildReqHeader(RequestHandler reqHandler) throws Exception {
        ...
        reqHandler.setParameter("transaction_id", stockoutOrderBO.getMerchantOrderNo());//财付通交易号
        ...
        reqHandler.setParameter("cert_type", stockoutOrderBO.getDeclarePayerCertType().toString()); //证件类型
        reqHandler.setParameter("cert_id", buyerIdNo); //证件号
        reqHandler.setParameter("name", buyerName);//姓名
        reqHandler.setParameter("action_type", TencentPayConstants.ActionType.ADD.getValue());//操作类型 1新增
    }
}
