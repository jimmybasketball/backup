...
public class FineExSkuSyncCommand extends WmsOrderSkuSyncCommand {
    ...
    private boolean syncMain(FineExSkuSynRequest request) {
        boolean flag = false;
        LogBetter.instance(logger).setMsg(warehouseBO.getName() + "同步请求商品及主码to FineEx参数")
            .addParm("request", request).setLevel(LogLevel.INFO).log();
        if (null == request) {
        ...
        try {
            ...
            String responseXml = CommonBizService.getInstance().fineExSend(
                "fineex.wms.products.sync", reqXml);
            ...
            if (null != responses && StringUtils.isNotBlank(responses.getFlag())
                && "true".equalsIgnoreCase(responses.getFlag())) {
                flag = true;
                logger.info("同步商品及主码到fineEx发网成功，返回");
            }
            LogBetter.instance(logger).setMsg(warehouseBO.getName() + "同步商品及主码响应FineEx参数")
                .addParm("request", request).setLevel(LogLevel.INFO).log();
            SkuWarehouseSyncLogManager skuWarehouseSyncLogManager = SpringBeanFactory.getBean(
                "skuWarehouseSyncLogManager", SkuWarehouseSyncLogManager.class);
            if (flag) {
                logger.info("同步商品及主码sku成功");
                DistributedLock distributedLock = SpringBeanFactory.getBean("distributedLock",
                    DistributedLock.class);
                try {
                    //防止并发插入
                    if (distributedLock.fetch("SKU-SYNC-LOCK")) {
                        SkuWarehouseSyncManager skuWarehouseSyncManager = SpringBeanFactory
                            .getBean("skuWarehouseSyncManager", SkuWarehouseSyncManager.class);
                        for (FineExSkuSynItem item : request.getItems()) {
                            ...
                            SkuWarehouseSyncDO warehouseSyncDO = skuWarehouseSyncManager
                                .getBySkuIdAndWarehouseId(Long.valueOf(item.getItemCode()),
                                    this.warehouseBO.getId());
                            if (warehouseSyncDO != null) {
                                warehouseSyncDO.setBarCode(item.getBarCode());
                                logger.info("skuId:{},warehouseId:{} 存在,update",
                                    item.getItemCode(), this.warehouseBO.getId());
                                if (WmsOperaterType.ADD.equals(getWmsOperaterType())) {
                                    warehouseSyncDO
                                        .setSyncState(SkuWarehouseSyncStateType.SYNC_SUCCESS.value);
                                } else {
                                    warehouseSyncDO
                                        .setSyncUpdateState(SkuWarehouseSyncStateType.SYNC_UPDATE_SUCCESS.value);
                                }
                                warehouseSyncDO.setGmtModified(new Date());
                                skuWarehouseSyncManager.update(warehouseSyncDO);
                            } else {
                                logger.error("skuId:{},warehouseId:{} 不存在", item.getItemCode(),
                                    this.warehouseBO.getId());
                            }
                            skuWarehouseSyncLogManager.createLog(Long.valueOf(item.getItemCode()),
                                this.warehouseBO.getId(), "成功", responses.toString(), 1);
                        }
                    }
                } catch (Exception e) {
                ...
                skuWarehouseSyncLogManager.createLog(0, this.warehouseBO.getId(), errorMessage,
                    responses.toString(), 0);
            }
        } catch (Exception e) {
    ...
    public boolean syncSub(FineExSkuSubSynRequest request) {
        ...
        try {
            ...
            String responseXml = CommonBizService.getInstance().fineExSend(
                "fineex.wms.product.subsync", reqXml);
            ...
            LogBetter.instance(logger).setMsg(warehouseBO.getName() + "同步商品子码响应FineEx参数")
                .addParm("request", request).setLevel(LogLevel.INFO).log();
            SkuWarehouseSyncLogManager skuWarehouseSyncLogManager = SpringBeanFactory.getBean(
                "skuWarehouseSyncLogManager", SkuWarehouseSyncLogManager.class);
            if (flag) {
                ...
                try {
                    ...
                    if (distributedLock.fetch("SKU-SYNC-LOCK")) {
                        ...
                        LogBetter.instance(logger).setMsg(warehouseBO.getName() + "插入子码同步记录成功,参数:")
                            .addParm("request", request).setLevel(LogLevel.INFO).log();
                        String reason = "skuId" + request.getSkuId() + ",bacode:"
                                        + request.getSubCode() + "成功";
                        skuWarehouseSyncLogManager.createLog(request.getSkuId(),
                            this.warehouseBO.getId(), reason, responses.toString(), 1);
                    }
                } catch (Exception e) {
                ...
                skuWarehouseSyncLogManager.createLog(request.getSkuId(), this.warehouseBO.getId(),
                    errorMessage, responses.toString(), 0);
            }
        } catch (Exception e) {
            errorMessage = e.getMessage();
            logger.error(errorMessage, e);
    ...
    private FineExSkuSynRequest buildSkuSynMainFineExRequest() {
        ...
        for (SkuEntity skuBean : skuEntities) {
            ...
            SkuWarehouseSyncManager skuWarehouseSyncManager = SpringBeanFactory.getBean(
                "skuWarehouseSyncManager", SkuWarehouseSyncManager.class);
            BaseQuery<SkuWarehouseSyncDO> bq = new BaseQuery<SkuWarehouseSyncDO>(
            ...
            if (null != skuWarehouseSyncDOs && skuWarehouseSyncDOs.size() > 0) {
                logger.info("此sku已经同步主码：skuId：" + skuBean.getId() + ";barCode:"
                            + skuWarehouseSyncDOs.get(0).getBarCode());
                continue;
            }
            ...
            SkuBarcodeManager skuBarcodeManager = SpringBeanFactory.getBean("skuBarcodeManager",
                SkuBarcodeManager.class);
            ...
            skuSynItem.setBarCode(skuBarcodeList.size() > 0 ? String.valueOf(skuBarcodeList.get(0)
                .getBarcode()) : null);
            ...
            String Property = sb.toString().length() > 100 ? sb.toString().substring(0, 99) : sb
                .toString();
    ...
    private List<FineExSkuSubSynRequest> buildSkuSynSubcodeFineExRequest() {
        ...
        for (SkuEntity skuEntity : skuEntities) {
            ...
            logger.info("同步子码，查询出的已经同步的skuWarehouseSyncDOs:"
                        + GsonUtil.getGsonInstance().toJson(skuWarehouseSyncDOs));
            ...
            if (null != barCodeList) {
                ...
                logger.info("移除已经同步的barcode后的barcode："
                            + GsonUtil.getGsonInstance().toJson(barCodeList));
            } else {
                return null;
            }
            ...
            for (String string : barCodeList) {
                ...
                sub.setBarCode(skuBarcodeList.size() > 0 ? String.valueOf(skuBarcodeList.get(0)
                    .getBarcode()) : null);
